...
public sealed class SearchEngineTokenizer : ISearchEngineTokenizer
{
    ...
    private readonly ConcurrentDictionary<DocId, TokenVectors> _generalDirectIndex;
    private readonly DirectIndexHandler _directIndexHandler;
    private readonly InvertedIndexHandler? _invertedIndexExtended;
    private readonly InvertedIndexHandler? _invertedIndexReduced;
    ...
    {
        _directIndexHandler = new DirectIndexHandler();
        _generalDirectIndex = _directIndexHandler.GetGeneralDirectIndex;
        _tokenizerProcessorFactory = tokenizerProcessorFactory;
        _searchType = searchType;

        if (_searchType != SearchType.Original)
        {
            _invertedIndexExtended = new InvertedIndexHandler();
            _invertedIndexReduced = new InvertedIndexHandler();
        }

        _searchProcessorFactory = new SearchProcessorFactory(
            _invertedIndexExtended,
            _invertedIndexReduced,
            _directIndexHandler,
    ...
    internal ConcurrentDictionary<DocId, TokenVectors> GetTokenVectors() => _generalDirectIndex;
        ...
        try
        {
            ...
            await foreach (var note in notes)
            {
                ...
                if (_searchType != SearchType.Original)
                {
                    ...
                    _invertedIndexExtended?.AddVector(extendedVector, noteDocId);
                    _invertedIndexReduced?.AddVector(reducedVector, noteDocId);
                }
    ...
    private static TokenVectors CreateTokensLine(ITokenizerProcessorFactory factory, TextRequestDto note)
    {
        ...
        return new TokenVectors(Extended: extendedTokenLine, Reduced: reducedTokenLine);
    }
