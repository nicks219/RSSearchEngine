...
* 6.0.0 `master` `на провреке: pre-1`
  ...
  * проблема: метрики не информативны, например, страница update идёт на ручку api/update для чтения, различить можно только по глаголам
  * решение: отрефакторить API, привести к единообразному виду
    * ✅ вынести пути контроллеров в константы - в контроллерах и тестах
    * ✅ написать билд-таску генерации json/tsx по cs файлу с константами
      * [x] todo: функционал и его зависимости необходимо удалять на CI, сейчас запуск настроен под Debug, под Release таска удаляется из сборки
      * [ ] todo: генерацию не надо запускать, если исходный файл с константами не поменялся
    * ✅ переместить пути из аннотаций контроллеров в ручки
    * ✅ объединить сегменты путей
    * ✅ написать использование json/tsx для получения констант на фронте: генерация либо вычитка
    * продумать рефакторинг: API должен быть понятным и единообразным
      * [ ] отрефакторить: CreateGetTagsUrl отличается от ReadGetTagsUrl наличием авторизации и завязан с recovery в клиенте
      * [x] добавить версионирование API
      * [x] в url заменить глаголы на существительные
      * [ ] в url убрать вложенность (надо рефакторить recovery на клиенте)
      * чеклист при замене HTTP глаголов: сервис, фронт, тесты, CORS
    * подумать: не обрабатывать исключения в managers, пробрасывать выше | Create.ReadStructuredTagList и Update.GetNoteWithTagsForUpdate
    * подумать: использовать статический логгер вместо ILogger<T> или ILoggerFactory в контроллерах
    * CI/CD: выглядит так, что надо использовать закомитанный файл с константами
  * ... 
