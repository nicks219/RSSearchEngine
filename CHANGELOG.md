...
#### в работе:
* permanent: частично доработана дополнительная сохранность данных
* upd: security обновление для js, версии пакетов подняты только в package-lock.json
* upd: добавлены vitest тесты на клиента
...
* **Permanent**: обеспечить сохранность данных даже при некорректных действиях пользователя
    * [ ] на стороне сервиса: добавить роли (чтение/+редактирование и создание в тч дампов/+удаление и рестор)
    * [x] на стороне сервиса: создавать дамп с новым именем на каждый новый текст, имена отдельной ротации отдавать в ответе
    * [x] на стороне клиента: добавить окно подтверждения
* **дополнительно**:
