...
* 6.0.0 `master` `не выкатывал`
  ...
  * k3s: билд self-contained под alpine:3.21 | глобализация отключена в csproj | используется для деплоя
  ...
  `в разработке`
  * WARN: не выкатывай пока не обложишь интеграционными тестами весь функционал
  * рефакторинг времени жизни служб
  * тяжелая для бд операция инициализации токенайзера теперь запускается строго однократно
  * тесты в IDE не ждут завершения очистки файлов, процесс очистки полностью отвязан
