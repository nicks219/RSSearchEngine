...
* 6.0.0 `master` `на провреке: 6.0.0-pre-1` `k6.0.0`
...
* `develop/release/6.0.0-pre-1` `пре-релиз` `k6.0.0`
...
--- 
* `рефакторинг` `development` `drafts`
  * [x] избавиться от лишних **nullable** в контексте
    * [x] разобраться с зависимостями контроллеров, управлять managers через DI
  * [x] рефакторинг IDataRepository:
  ...
  * [x] рефакторинг dto и code-style: 
    * [x] нейминг: managers -> services, убрать request/response из dto 
    * [x] распилить dto CatalogItem по слоям
    * [x] сделать dto неизменяемыми (init-only либо primary-конструктор)
    * [ ] перенести в маппинг trim из CreateManager, checked/unchecked сделать bool
    * [ ] единообразно отсортировать методы
  * [ ] рефакторинг tokenizer service: 
    * [x] сделать обертку с двумя векторами для каждой заметки
    * [ ] сделать фабрику для tokenizer processor
  * [ ] выделить domain в отдельный проект:
    * [ ] поправить билд докера с учетом изменений

* `fix`
  * [x] fix: исправлены перепутанные поля old/new в UpdateCredos
    * [!] todo: написать тест на UpdateCredos
  * [x] fix: исправлено получение scoped-зависимости в singleton миграторе
---
