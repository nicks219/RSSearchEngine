...
* `develop/release/6.0.0-pre-2` `рефакторинг` `пре-релиз` `beta`
  ...
  * `d0.0.1` `master` upd: рассмотреть возможность заменить в Rsse.Data конкретные зависимости на абстрактные
* `develop/release/6.0.0-pre-3` `OTLP` `пре-релиз` `3`
  * upd: добавить манифест для скрейпа и экспорта метрик **otel** - **graphana**, также см [kubectl create secret](.common/scripts.k3s.bash)
  * upd: `k3s: *.v2.yml` добавить хелсчеки (трафик можно принимать при завершения инициализации)
  ...
  * upd: `k6.0.0` `k3s: *.v2.yml` поставлять observability в OTLP формате на `http://otel-collector:4317`
