...
* `разработка`
  * избавиться от паразитного трафика (убрать шум в сигналах observability)
    - [x] остановить `rsse-app-ingress-http`: т.е отрезать весь трафик, кроме https://notefinder.ru (IP и silversword)
    - [ ] настроить редирект http -> https (проверить `rsse-app-ingress-https-ru-https-only`)
