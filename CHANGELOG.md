...
* 6.0.0 `master` `на провреке: pre-1`
  ...
  * решение: отрефакторить API, привести к единообразному виду
    ...
    * ✅ написать билд-таску генерации json по cs файлу с константами, костыль: отрабатывает на Debug, но код не получилось разместить под #if
    * ✅ переместить пути из аннотаций контроллеров в ручки
    * ✅ объединить сегменты путей
    * написать использование json для получения констант на фронте: генерация либо вычитка
    * продумать рефакторинг: API должен быть понятным и единообразным - возможно, перенести читающие ручки в read
    * интегрировать кодогенерацию с CI/CD
  * ... 
