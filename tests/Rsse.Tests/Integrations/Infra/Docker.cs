...
public abstract class Docker
{
    ...
    public static async Task CleanUpDbContainers(CancellationToken ct)
    {
        ...
        await RunDockerCli(args, ct).ConfigureAwait(false);
        ...
        string command, string expected, CancellationToken ct)
    {
        ...
        process.Start();
        var processOut = await process.StandardOutput.ReadToEndAsync(ct).ConfigureAwait(false);
        Console.WriteLine(processOut);
        Console.WriteLine(await process.StandardError.ReadToEndAsync(ct).ConfigureAwait(false));
        ...
        while (count-- > 0)
        {
            ...
            var output = await healthcheckProcess.StandardOutput.ReadToEndAsync(ct).ConfigureAwait(false);
